...
export type MediaThunk = ThunkAction<
  void,
  MediaState,
  null,
  Action<SearchAction>
...
export const fetchArtistByTerm =
  (): MediaThunk => async (dispatch: DispatchType, getState) => {
    const {
      searchTerm,
      offsetArtist,
      searchByArtist,
    ...
    if (searchByArtist && length === 0) {
      const response = await serviceAPI.getMediaResourceArtistByTerm(
        searchTerm,
        offsetArtist,
        10
      );
...
export const fetchAlbumByTerm =
  (): MediaThunk => async (dispatch: DispatchType, getState) => {
    ...
    if (searchByAlbum && length === 0) {
      const response = await serviceAPI.getMediaResourceAlbumByTerm(
        searchTerm,
        offsetAlbum,
        10
      );
...
export const fetchSongByTerm =
  (): MediaThunk => async (dispatch: DispatchType, getState) => {
    ...
    if (searchBySong && length === 0) {
      const response = await serviceAPI.getMediaResourceSongByTerm(
        searchTerm,
        offsetSong,
        10
      );
...
export const loadResultsIntoCollection =
  (): MediaThunk => async (dispatch: DispatchType, getState) => {
    ...
    while (count > 0) {
      const singleSearchTypeList = getCurrentListType(
        listArtist,
        listAlbum,
        listSong
      );
...
export const fetchResultsByTypes =
  (): MediaThunk => async (dispatch: DispatchType, getState) => {
...
export const fetchLoadResults =
  (): MediaThunk => (dispatch: DispatchType, getState) => {
    return dispatch(fetchResultsByTypes()).then(() =>
      dispatch(loadResultsIntoCollection())
    );
  };

export const defineSearchByTerm =
  (term: string): MediaThunk =>
